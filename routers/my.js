    ...
    fs.readFile(path, "binary", function (err, file) {
      if (err) {
        ...
        fs.writeFile("public/img/" + filename, file, "binary", (err) => {
          ...
          const avatarUrl = "http://localhost:3001/public/img/" + filename;
          if (req.session.userInfo) {
            const { _id } = req.session.userInfo;
            User.findOne({ _id }, (err, docs) => {
              ...
              const { username, avatar } = docs;
              User.updateOne({ username, avatar: avatarUrl }).then(() => {
                if (avatar !== "http://localhost:3001/public/img/avatar.jpg") {
                  const oldAvatar = avatar.replace(
                    "http://localhost:3001/",
                    ""
                  );
